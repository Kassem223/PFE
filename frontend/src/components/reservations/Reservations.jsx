...
export const Reservations = () => {
  ...
  const fetchUserReservations = async (userId) => {
    try {
      ...
      response.data.forEach(reservation => {
        ...
        groupedReservations[key].all_ids.push(reservation.id_reservation);
      });
  ...
  const fetchInvitations = async (reservation, groupKey) => {
    // Find all reservation IDs in this group
    const groupIds = reservations
      .filter(res => getReservationGroupKey(res) === groupKey)
  ...
  const toggleReservationDetails = (reservation) => {
    ...
    if (expandedReservation === groupKey) {
      ...
      if (!equipments[groupKey]) {
        fetchEquipments(reservation.id_reservation, groupKey);
      }
    }
  };
  ...
  const getAcceptedCount = (groupKey) => {
    ...
    return groupInvitations.filter(invitation => invitation.statut === 'acceptée').length;
  };
      ...
      <main className="p-8 max-w-7xl mx-auto w-full">
        <div className="space-y-8">
          ...
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-3 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-full border border-blue-200 dark:border-blue-800">
              <Calendar className="w-5 h-5 text-blue-600 dark:text-blue-400" />
              <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Mes Réservations</span>
            </div>
            <h1 className="text-5xl font-bold text-slate-900 dark:text-white mb-4">
              Gestion de vos réservations
            </h1>
            <p className="text-slate-600 dark:text-slate-400 text-lg max-w-2xl mx-auto">
              Suivez, gérez et organisez toutes vos réservations d'équipements en un seul endroit
            </p>
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {expandedReservation === getReservationGroupKey(reservation) && (
                      <div className="border-t border-slate-200/60 dark:border-slate-800/60 pt-6 space-y-6">
                          ...
                          <div>
                            ...
                            {equipments[getReservationGroupKey(reservation)]?.length > 0 ? (
                              <div className="space-y-3">
                                {/* Afficher les équipements supplémentaires */}
                                {equipments[getReservationGroupKey(reservation)]?.map((equipment, index) => (
                                  <div key={index} className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 dark:from-blue-400 dark:to-blue-500 rounded-full flex items-center justify-center">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <p className="text-sm text-slate-500 dark:text-slate-400">
                                        <span className="font-semibold">ID:</span> {equipment.id}
                                      </p>
                                    </div>
                                  </div>
                                ))}
                              ...
                              <div className="text-center py-8 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                                <Package className="w-12 h-12 text-slate-400 mx-auto mb-3" />
                                <p className="text-slate-600 dark:text-slate-400">
                                  Aucun équipement supplémentaire pour cette réservation
                                </p>
                              </div>
                            )}
                          </div>
                          ...
                          {invitations[getReservationGroupKey(reservation)] && invitations[getReservationGroupKey(reservation)].length > 0 ? (
                            <div className="space-y-3">
                              {invitations[getReservationGroupKey(reservation)].map((invitation, index) => (
                                <div key={index} className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                                  <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-600 dark:from-purple-400 dark:to-purple-500 rounded-full flex items-center justify-center">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="space-y-2">
                                      <p className="text-sm text-slate-500 dark:text-slate-400">
                                        <span className="font-semibold">Statut:</span> {invitation.status}
                                      </p>
                                      <p className="text-sm text-slate-500 dark:text-slate-400">
                                        <span className="font-semibold">Invité le:</span> {new Date(invitation.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}
                                      </p>
                                      {invitation.accepted_at && (
                                        <p className="text-sm text-slate-500 dark:text-slate-400">
                                          <span className="font-semibold">Répondu le:</span> {new Date(invitation.accepted_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}
                                        </p>
                                      )}
                                      {invitation.declined_at && (
                                        <p className="text-sm text-slate-500 dark:text-slate-400">
                                          <span className="font-semibold">Réponse:</span> {invitation.statut === 'refusée' ? 'Refusé' : 'En attente'}
                                        </p>
                                      )}
                                    </div>
                                  </div>
                            ...
                            <div className="text-center py-8 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                              ...
                              <p className="text-slate-600 dark:text-slate-400">
                                Aucune invitation pour cette réservation
                              </p>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Créée le</p>
                            <p className="font-semibold text-slate-900 dark:text-white">
                              {new Date(reservation.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </p>
                          </div>
                          <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Statut actuel</p>
